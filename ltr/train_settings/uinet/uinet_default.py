...
from ltr.dataset import Lasot, TrackingNet, MSCOCOSeq,MSCOCOVal
from ltr.data import processing, sampler, LTRLoader
import ltr.models.masknet.uinet as uinet
...
def run(settings):
    ...
    settings.description = 'UInet with default settings.'
    ...
    data_processing_train = processing.UInet2Processing(search_area_factor=settings.search_area_factor,
    ...
    data_processing_val = processing.UInet2Processing(search_area_factor=settings.search_area_factor,
                                                    output_sz=settings.output_sz,
                                                    center_jitter_factor=settings.center_jitter_factor,
                                                    scale_jitter_factor=settings.scale_jitter_factor,
                                                    mode='sequence',
    ...
    dataset_train = sampler.UISampler([coco_train], [1],
    ...
    dataset_val = sampler.UISampler([coco_val], [1], samples_per_epoch=500*settings.batch_size, max_gap=50,processing=data_processing_val)
    ...
    # Create network
    net = uinet.uinet_resnet18(backbone_pretrained=True)

    # Set objective
    objective = nn.BCELoss()

    # Create actor, which wraps network and objective
    actor = actors.UIActor(net=net, objective=objective)
    ...
    trainer.train(500, load_latest=True, fail_safe=False)
